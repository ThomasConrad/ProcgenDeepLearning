...
class Storage():
	...
	## KK Mixreg implementation (experimental)
	def mix_augment(self, increase=2, alpha=1):
		aug_length = self.num_steps*(increase-1)
		last_index = self.num_steps-1

		# Increase size of tensors to hold increased amounts
		if self.obs.size()[0] <= (self.num_steps+1):
			self.obs = torch.cat((self.obs, torch.zeros(aug_length, self.num_envs, *self.obs_shape)),0)
			self.action = torch.cat((self.action, torch.zeros(aug_length, self.num_envs)),0)
			self.reward = torch.cat((self.reward, torch.zeros(aug_length, self.num_envs)),0)
			self.done = torch.cat((self.done, torch.zeros(aug_length, self.num_envs)),0)
			self.log_prob = torch.cat((self.log_prob, torch.zeros(aug_length, self.num_envs)),0)
			self.value = torch.cat((self.value, torch.zeros(aug_length, self.num_envs)),0)
			self.returns = torch.cat((self.returns, torch.zeros(aug_length, self.num_envs)),0)
			self.advantage = torch.cat((self.advantage, torch.zeros(aug_length, self.num_envs)),0)

		for i in range(aug_length):
			# Sample lambda value from beta distribution
			tmp_lambda = np.random.beta(alpha, alpha)

			# Pick two random different indices
			rand_a = randint(0,last_index)
			rand_b = rand_a
			while rand_b == rand_a:
				rand_b = randint(0,last_index)
			
			#Mix the different observations and associated values according to lambda
			tmp_action		= self.action[rand_a]	if tmp_lambda >= 0.5 else self.action[rand_b]
			tmp_log_prob	= self.log_prob[rand_a]	if tmp_lambda >= 0.5 else self.log_prob[rand_b]
			tmp_done		= self.done[rand_a]		if tmp_lambda >= 0.5 else self.done[rand_b]
			tmp_obs =		tmp_lambda*self.obs[rand_a]			+ (1 - tmp_lambda)*self.obs[rand_b]
			tmp_value =		tmp_lambda*self.value[rand_a]		+ (1 - tmp_lambda)*self.value[rand_b]
			tmp_returns =	tmp_lambda*self.returns[rand_a]		+ (1 - tmp_lambda)*self.returns[rand_b]
			tmp_advantage =	tmp_lambda*self.advantage[rand_a]	+ (1 - tmp_lambda)*self.advantage[rand_b]

			#Fill out the blank spaces
			self.obs[last_index + i + 2]		= tmp_obs
			self.action[last_index + i + 1]		= tmp_action
			self.log_prob[last_index + i + 1]	= tmp_log_prob
			self.done[last_index + i + 1]		= tmp_done
			self.value[last_index + i + 2]		= tmp_value
			self.returns[last_index + i + 1]	= tmp_returns
			self.advantage[last_index + i + 1]	= tmp_advantage

	def get_mix_generator(self, increase=2, alpha=1, batch_size=1024):
		self.mix_augment(increase)

		iterator = BatchSampler(SubsetRandomSampler(range(self.num_steps*increase*self.num_envs)), batch_size, drop_last=True)
		for indices in iterator:
			obs = self.obs[:-1].reshape(-1, *self.obs_shape)[indices].cuda()
			action = self.action.reshape(-1)[indices].cuda()
			log_prob = self.log_prob.reshape(-1)[indices].cuda()
